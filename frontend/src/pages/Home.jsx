...
function Home() {
  ...
  // Decode the JWT payload to grab the logged-in user's ID
  const currentUserId = React.useMemo(() => {
    if (!accessToken) return null;
    try {
      const payload = JSON.parse(atob(accessToken.split('.')[1]));
      return payload.id; // Check your backend to see if this is 'userId' or just 'id'
    } catch (e) {
      return null;
    }
  }, [accessToken]);
              ...
              <div key={post._id} className="bg-white border border-gray-100 rounded-2xl shadow-sm p-5 flex flex-col gap-3 transition-all hover:border-gray-200 animate-in fade-in duration-150 w-full">
                ...
                <div className="flex items-center gap-4 pt-2 border-t border-gray-50">
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 text-xs font-bold transition-colors cursor-pointer group ${hasLiked ? "text-pink-600" : "text-gray-500 hover:text-pink-600"
                      }`}
                  >
                    <ThumbsUp
                      className={`w-4 h-4 transition-transform group-hover:scale-105 ${hasLiked ? "fill-pink-600 text-pink-600" : ""
                        }`}
                    />
                    {totalLikes}
                  </button>
                  ...
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 text-xs font-bold transition-colors cursor-pointer group ${hasDisliked ? "text-red-600" : "text-gray-500 hover:text-red-600"
                      }`}
                  >
                    <ThumbsDown
                      className={`w-4 h-4 transition-transform group-hover:scale-105 ${hasDisliked ? "fill-red-600 text-red-600" : ""
                        }`}
                    />
                    {totalDislikes}
                  </button>
